...
// Post meta.
add_action( 'added_post_meta', 'wp_cache_set_posts_last_changed', 10, 0 );
add_action( 'updated_post_meta', 'wp_cache_set_posts_last_changed', 10, 0 );
add_action( 'deleted_post_meta', 'wp_cache_set_posts_last_changed', 10, 0 );
...
add_action( 'init', 'wp_init_targeted_link_rel_filters', 10, 0 );
...
foreach (
	...
	add_action( $action, '_delete_option_fresh_site', 0, 0 );
}
...
add_action( 'auth_cookie_malformed', 'rest_cookie_collect_status', 10, 0 );
add_action( 'auth_cookie_expired', 'rest_cookie_collect_status', 10, 0 );
add_action( 'auth_cookie_bad_username', 'rest_cookie_collect_status', 10, 0 );
add_action( 'auth_cookie_bad_hash', 'rest_cookie_collect_status', 10, 0 );
add_action( 'auth_cookie_valid', 'rest_cookie_collect_status', 10, 0 );
...
add_action( 'wp_head', 'wlwmanifest_link', 10, 0 );
add_action( 'wp_head', 'locale_stylesheet', 10, 0 );
...
add_action( 'wp_print_footer_scripts', '_wp_footer_scripts', 10, 0 );
...
// Login actions.
add_action( 'login_head', 'wp_robots', 1, 0 );
add_filter( 'login_head', 'wp_resource_hints', 8, 0 );
add_action( 'login_head', 'wp_print_head_scripts', 9, 0 );
add_action( 'login_head', 'print_admin_styles', 9, 0 );
add_action( 'login_head', 'wp_site_icon', 99, 0 );
add_action( 'login_footer', 'wp_print_footer_scripts', 20, 0 );
...
if ( ! defined( 'DOING_CRON' ) ) {
	add_action( 'init', 'wp_cron', 10, 0 );
}

// HTTPS detection.
add_action( 'init', 'wp_schedule_https_detection', 10, 0 );
...
add_action( 'sanitize_comment_cookies', 'sanitize_comment_cookies', 10, 0 );
add_action( 'init', 'smilies_init', 5, 0 );
add_action( 'plugins_loaded', 'wp_maybe_load_widgets', 0, 0 );
add_action( 'plugins_loaded', 'wp_maybe_load_embeds', 0, 0 );
...
add_action( 'comment_form', 'wp_comment_form_unfiltered_html_nonce', 10, 0 );
...
add_action( 'init', 'wp_schedule_delete_old_privacy_export_files', 10, 0 );
...
// Redirect old slugs.
add_action( 'template_redirect', 'wp_old_slug_redirect', 10, 0 );
...
add_action( 'init', '_show_post_preview', 10, 0 );

// Output JS to reset window.name for previews.
add_action( 'wp_head', 'wp_post_preview_js', 1, 0 );

// Timezone.
add_filter( 'pre_option_gmt_offset', 'wp_timezone_override_offset', 10, 0 );

// If the upgrade hasn't run yet, assume link manager is used.
add_filter( 'default_option_link_manager_enabled', '__return_true', 10, 0 );

// This option no longer exists; tell plugins we always support auto-embedding.
add_filter( 'pre_option_embed_autourls', '__return_true', 10, 0 );
...
// REST API actions.
add_action( 'init', 'rest_api_init', 10, 0 );
add_action( 'rest_api_init', 'rest_api_default_filters', 10, 0 );
add_action( 'rest_api_init', 'register_initial_settings', 10, 0 );
add_action( 'rest_api_init', 'create_initial_rest_routes', 99, 0 );
add_action( 'parse_request', 'rest_api_loaded', 10, 0 );

// Sitemaps actions.
add_action( 'init', 'wp_sitemaps_get_server', 10, 0 );

/**
 * Filters formerly mixed into wp-includes.
 */
// Theme.
add_action( 'setup_theme', 'create_initial_theme_features', 0, 0 );
add_action( 'setup_theme', '_add_default_theme_supports', 1, 0 );
add_action( 'wp_loaded', '_custom_header_background_just_in_time', 10, 0 );
add_action( 'wp_head', '_custom_logo_header_styles', 10, 0 );
...
add_action( 'save_post', 'delete_get_calendar_cache', 10, 0 );
add_action( 'delete_post', 'delete_get_calendar_cache', 10, 0 );
add_action( 'update_option_start_of_week', 'delete_get_calendar_cache', 10, 0 );
add_action( 'update_option_gmt_offset', 'delete_get_calendar_cache', 10, 0 );

// Author.
add_action( 'transition_post_status', '__clear_multi_author_cache', 10, 0 );
...
// KSES.
add_action( 'init', 'kses_init', 10, 0 );
add_action( 'set_current_user', 'kses_init', 10, 0 );
...
add_action( 'wp_enqueue_scripts', 'wp_enqueue_classic_theme_styles', 10, 0 );
...
add_action( 'enqueue_block_assets', 'wp_enqueue_registered_block_scripts_and_styles', 10, 0 );
add_action( 'enqueue_block_assets', 'enqueue_block_styles_assets', 30, 0 );
add_action( 'enqueue_block_editor_assets', 'wp_enqueue_registered_block_scripts_and_styles', 10, 0 );
add_action( 'enqueue_block_editor_assets', 'enqueue_editor_block_styles_assets', 10, 0 );
add_action( 'enqueue_block_editor_assets', 'wp_enqueue_editor_block_directory_assets', 10, 0 );
add_action( 'enqueue_block_editor_assets', 'wp_enqueue_editor_format_library_assets', 10, 0 );
add_action( 'enqueue_block_editor_assets', 'wp_enqueue_global_styles_css_custom_properties', 10, 0 );
...
add_action( 'wp_enqueue_scripts', 'wp_enqueue_global_styles', 10, 0 );
add_action( 'wp_footer', 'wp_enqueue_global_styles', 1, 0 );

// Global styles custom CSS.
add_action( 'wp_enqueue_scripts', 'wp_enqueue_global_styles_custom_css', 10, 0 );
...
add_action( 'wp_head', 'wp_maybe_inline_styles', 1, 0 ); // Run for styles enqueued in <head>.
add_action( 'wp_footer', 'wp_maybe_inline_styles', 1, 0 ); // Run for late-loaded styles in the footer.
...
// Taxonomy.
add_action( 'init', 'create_initial_taxonomies', 0, 0 ); // Highest priority.
add_action( 'change_locale', 'create_initial_taxonomies', 10, 0 );

// Canonical.
add_action( 'template_redirect', 'redirect_canonical' );
add_action( 'template_redirect', 'wp_redirect_admin_locations', 1000, 0 );
...
add_action( 'plugins_loaded', '_wp_add_additional_image_sizes', 0, 0 );
...
// Widgets.
add_action( 'after_setup_theme', 'wp_setup_widgets_block_editor', 1, 0 );
add_action( 'init', 'wp_widgets_init', 1, 0 );
add_action( 'change_locale', array( 'WP_Widget_Media', 'reset_default_labels' ) );
add_action( 'widgets_init', '_wp_block_theme_register_classic_sidebars', 1, 0 );
...
// Embeds.
add_action( 'rest_api_init', 'wp_oembed_register_route', 10, 0 );
add_filter( 'rest_pre_serve_request', '_oembed_rest_pre_serve_request', 10, 4 );

add_action( 'wp_head', 'wp_oembed_add_discovery_links', 10, 0 );
add_action( 'wp_head', 'wp_oembed_add_host_js', 10, 0 ); // Back-compat for sites disabling oEmbed host JS by removing action.
...
add_action( 'embed_head', 'wp_robots', 10, 0 );
add_action( 'embed_head', 'rel_canonical', 10, 0 );
add_action( 'embed_head', 'locale_stylesheet', 30, 0 );
...
add_action( 'embed_footer', 'wp_print_footer_scripts', 20, 0 );
...
add_action( 'wp_footer', 'the_block_template_skip_link', 10, 0 );
add_action( 'setup_theme', 'wp_enable_block_templates', 10, 0 );
add_action( 'wp_loaded', '_add_template_loader_filters', 10, 0 );

// Fluid typography.
add_filter( 'render_block', 'wp_render_typography_support', 10, 2 );

// User preferences.
add_action( 'init', 'wp_register_persisted_preferences_meta', 10, 0 );
